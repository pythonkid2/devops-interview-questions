# Wipro

[Home](../README.md) > [Companies](./README.md) > **Wipro**

## Overview

| Metric | Value |
| --- | ---: |
| Questions | 11 |
| Topics covered | 3 |

## Topics asked

- [General](../topics/general.md) — 7
- [Azure](../topics/azure.md) — 3
- [System Design](../topics/system-design.md) — 1

## Most repeated

- [Spark performance optimization](../topics/general.md#spark-performance-optimization) — 3× · General
- [Delta Lake vs Data Lake](../topics/general.md#delta-lake-vs-data-lake) — 2× · General
- [Apache Spark architecture & Catalyst Optimizer](../topics/system-design.md#apache-spark-architecture-catalyst-optimizer) — 1× · System Design
- [Azure Blob Storage vs ADLS Gen2](../topics/azure.md#azure-blob-storage-vs-adls-gen2) — 1× · Azure
- [Azure Data Factory scenario-based questions](../topics/azure.md#azure-data-factory-scenario-based-questions) — 1× · Azure
- [Azure Databricks & Unity Catalog](../topics/azure.md#azure-databricks-unity-catalog) — 1× · Azure
- [End-to-end project discussion & real-world scenarios](../topics/general.md#end-to-end-project-discussion-real-world-scenarios) — 1× · General
- [PySpark DataFrame coding scenarios](../topics/general.md#pyspark-dataframe-coding-scenarios) — 1× · General
- [SCD Type 2 implementation](../topics/general.md#scd-type-2-implementation) — 1× · General
- [Spark join strategies & lazy evaluation](../topics/general.md#spark-join-strategies-lazy-evaluation) — 1× · General
- [SQL (CTE, Window Functions & Joins)](../topics/general.md#sql-cte-window-functions-joins) — 1× · General

## By topic

### Azure

- [Azure Blob Storage vs ADLS Gen2](../topics/azure.md#azure-blob-storage-vs-adls-gen2) — Medium
- [Azure Data Factory scenario-based questions](../topics/azure.md#azure-data-factory-scenario-based-questions) — Medium
- [Azure Databricks & Unity Catalog](../topics/azure.md#azure-databricks-unity-catalog) — Medium

### General

- [Delta Lake vs Data Lake](../topics/general.md#delta-lake-vs-data-lake) — Medium
- [End-to-end project discussion & real-world scenarios](../topics/general.md#end-to-end-project-discussion-real-world-scenarios) — Hard
- [PySpark DataFrame coding scenarios](../topics/general.md#pyspark-dataframe-coding-scenarios) — Medium
- [SCD Type 2 implementation](../topics/general.md#scd-type-2-implementation) — Medium
- [Spark join strategies & lazy evaluation](../topics/general.md#spark-join-strategies-lazy-evaluation) — Medium
- [Spark performance optimization](../topics/general.md#spark-performance-optimization) — Medium
- [SQL (CTE, Window Functions & Joins)](../topics/general.md#sql-cte-window-functions-joins) — Medium

### System Design

- [Apache Spark architecture & Catalyst Optimizer](../topics/system-design.md#apache-spark-architecture-catalyst-optimizer) — Hard

## Recently added

- [Delta Lake vs Data Lake](../topics/general.md#delta-lake-vs-data-lake) — 2026-08-13
- [Spark performance optimization](../topics/general.md#spark-performance-optimization) — 2026-08-12
- [End-to-end project discussion & real-world scenarios](../topics/general.md#end-to-end-project-discussion-real-world-scenarios) — 2026-07-27
- [Azure Blob Storage vs ADLS Gen2](../topics/azure.md#azure-blob-storage-vs-adls-gen2) — 2026-07-27
- [SCD Type 2 implementation](../topics/general.md#scd-type-2-implementation) — 2026-07-27
- [PySpark DataFrame coding scenarios](../topics/general.md#pyspark-dataframe-coding-scenarios) — 2026-07-27
- [SQL (CTE, Window Functions & Joins)](../topics/general.md#sql-cte-window-functions-joins) — 2026-07-27
- [Spark join strategies & lazy evaluation](../topics/general.md#spark-join-strategies-lazy-evaluation) — 2026-07-27
- [Apache Spark architecture & Catalyst Optimizer](../topics/system-design.md#apache-spark-architecture-catalyst-optimizer) — 2026-07-27
- [Azure Databricks & Unity Catalog](../topics/azure.md#azure-databricks-unity-catalog) — 2026-07-27

[← All companies](./README.md) · [Home](../README.md)
